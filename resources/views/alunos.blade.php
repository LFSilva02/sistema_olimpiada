...
<div class="container mx-auto px-4">
    @foreach($turmas->groupBy('serie') as $serie => $turmasPorSerie)
        ...
        @foreach($turmasPorSerie as $turma)
            <h3 class="text-xl font-semibold mt-4 mb-2 cursor-pointer text-blue-500 hover:underline" onclick="window.location='{{ route('turmas.alunos', $turma->id) }}'">Turma: {{ $turma->nome_turma }}</h3>
        @endforeach
    @endforeach
</div>
