        ...
        <div class="flex flex-wrap justify-center">
            @foreach ($turmasSerie as $turma)
                <div class="flex justify-center mb-4 mr-4">
                    <div
                        class="turma-card bg-{{ $turma->ativo ? 'blue-300' : 'gray-300' }} rounded-md px-4 py-2 flex items-center">
                        <p class="text-center cursor-pointer openStudentFormButton" data-turma-id="{{ $turma->id }}">
                            {{ $turma->nome_turma }}</p>
                        <div class="flex items-center">
                            <button type="button"
                                class="hover:bg-blue-300 text-black hover:text-white btn btn-outline-primary me-2 px-5 openEditFormButton"
                                data-id="{{ $turma->id }}" data-nome="{{ $turma->nome_turma }}"
                                data-serie="{{ $turma->serie }}" data-ano="{{ $turma->ano }}">
                            ...
                            <form action="{{ route('turmas.inativar') }}" method="POST" class="inline">
                                ...
                                <button type="submit"
                                    class="btn btn-outline-danger pr-5 hover:bg-blue-300 text-black hover:text-white">
                                    <i class="bi bi-trash"></i>
                                </button>
                            </form>
                            @if (!$turma->ativo)
                                <form action="{{ route('turmas.ativar', ['id' => $turma->id]) }}" method="POST"
                                    class="inline">
                                    @csrf
                                    @method('PUT')
