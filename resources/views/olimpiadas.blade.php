...
<div class="ml-64 pt-20 mb-8 flex items-center justify-between">
  ...
  <div class="mr-4">
    <button id="openFormButton" class="bg-[#134196] hover:bg-blue-300 text-white hover:text-black font-bold py-2 px-4 rounded">Cadastrar Olimpíada</button>
  </div>
</div>
...
<div class="flex justify-center">
  <div class="w-full lg:w-2/3 px-4">
    @foreach($olimpiadas as $olimpiada)
      <div class="olimpiada-card {{ $olimpiada->ativo ? 'bg-blue-300' : 'inactive' }} rounded-md px-4 py-2 mb-4 flex items-center relative">
        <button type="button" class="focus:outline-none">
          <p class="text-center cursor-pointer">{{ $olimpiada->nome_olimpiada }}</p>
        </button>
        <div class="flex items-center ml-auto">
          {{-- botão para editar --}}
          <button class="openEditFormButton bi bi-pencil mx-3" data-id="{{ $olimpiada->id }}" data-nome="{{ $olimpiada->nome_olimpiada }}" data-data="{{ $olimpiada->data_olimpiada }}" data-horario="{{ $olimpiada->horario }}" data-local="{{ $olimpiada->local }}" data-sala="{{ $olimpiada->sala }}"></button>
  ...
  <div class="bg-white p-8 rounded-lg shadow-lg w-1/2 relative">
    ...
    <form id="form" action="{{ route('olimpiadas.store') }}" method="POST">
      ...
      <div class="mb-4">
        ...
        <input type="text" name="nome_olimpiada" id="nomeOlimpiada" class="mt-1 p-2 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
      </div>
      <div class="grid grid-cols-2 gap-4 mb-4">
        <div>
          <label for="dataOlimpiada" class="block text-sm font-medium text-gray-700">Data da Olimpíada</label>
          <input type="date" name="data_olimpiada" id="dataOlimpiada" class="mt-1 p-2 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        </div>
        <div>
          <label for="horario" class="block text-sm font-medium text-gray-700">Horário</label>
          <input type="time" name="horario" id="horario" class="mt-1 p-2 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        </div>
      </div>
      <div class="grid grid-cols-2 gap-4 mb-4">
        <div>
          <label for="local" class="block text-sm font-medium text-gray-700">Local</label>
          <input type="text" name="local" id="local" class="mt-1 p-2 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        </div>
        <div>
          <label for="sala" class="block text-sm font-medium text-gray-700">Sala</label>
          <input type="text" name="sala" id="sala" class="mt-1 p-2 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        </div>
      </div>
      <div class="form-footer">
        <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Salvar</button>
        <button type="button" id="closeFormButton" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">Cancelar</button>
      </div>
    </form>
  </div>
</div>
...
<div id="editFormContainer" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
  <div class="bg-white p-8 rounded-lg shadow-lg w-1/2 relative">
    ...
    <form id="editForm" method="POST">
      ...
      @method('PUT')
      <div class="mb-4">
        <label for="editNomeOlimpiada" class="block text-sm font-medium text-gray-700">Nome da Olimpíada</label>
        <input type="text" name="nome_olimpiada" id="editNomeOlimpiada" class="mt-1 p-2 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
      </div>
      <div class="grid grid-cols-2 gap-4 mb-4">
        <div>
          <label for="editDataOlimpiada" class="block text-sm font-medium text-gray-700">Data da Olimpíada</label>
          <input type="date" name="data_olimpiada" id="editDataOlimpiada" class="mt-1 p-2 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        </div>
        <div>
          <label for="editHorario" class="block text-sm font-medium text-gray-700">Horário</label>
          <input type="time" name="horario" id="editHorario" class="mt-1 p-2 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        </div>
      </div>
      <div class="grid grid-cols-2 gap-4 mb-4">
        <div>
          <label for="editLocal" class="block text-sm font-medium text-gray-700">Local</label>
          <input type="text" name="local" id="editLocal" class="mt-1 p-2 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        </div>
        <div>
          <label for="editSala" class="block text-sm font-medium text-gray-700">Sala</label>
          <input type="text" name="sala" id="editSala" class="mt-1 p-2 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        </div>
      </div>
      <div class="form-footer">
        <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Salvar</button>
        <button type="button" id="closeEditFormButton" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">Cancelar</button>
      </div>
    </form>
  </div>
</div>
